...
import AnsiRobot.modules.sql.users_sql as sql
from AnsiRobot import (
    BOT_NAME,
    BOT_USERNAME,
    LOGGER,
    OWNER_ID,
...
from AnsiRobot.modules import ALL_MODULES
from AnsiRobot.modules.helper_funcs.chat_status import is_user_admin
from AnsiRobot.modules.helper_funcs.misc import paginate_modules
...
∂ση'т αѕк ƒσя мυѕι¢ ƒєтυяєѕ😍........в¢z тσυ¢н ση мυѕι¢ вυттση♥️ αη∂ ¢нє¢к мυѕι¢ ¢σммαη∂ѕ💯
♥️╔═════ஜ۩۞۩ஜ════╗♥️
      
...
♥️╚═════ஜ۩۞۩ஜ════╝♥️
──────────────────
*๏ ¢ℓι¢к ση ♥️ℓσνє♥️ вυттση тσ gєт ιηƒσямαтισηѕ αвσυт тнιѕ вσт.* 
...
buttons = [
    ...
    [
        ...
        InlineKeyboardButton(text="♥️мυѕι¢♥️", callback_data="Music_"),
    ],
    [
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("AnsiRobot.modules." + module_name)
    ...
    if update.effective_chat.type == "private":
        if len(args) >= 1:
            ...
            x=update.effective_message.reply_sticker(
                "CAACAgEAAxkBAAEInedkPVe6FrIfrIyFMWcw8__M3BfLCQAC4wADUSkNORemx3nCkOSbLwQ")
            x.delete()
...
@run_async
def Ansi_about_callback(update: Update, context: CallbackContext):
    query = update.callback_query
    if query.data == "Ansi_":
        uptime = get_readable_time((time.time() - StartTime))
        query.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                   ...
                   [
                        InlineKeyboardButton(text="♥️", callback_data="Ansi_back"),
                    ],
                ]
            ),
        )
    elif query.data == "Ansi_support":
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        ...
                        InlineKeyboardButton(
                            text="💗ᴜᴩᴅᴀᴛᴇs💓", url=f"https://t.me/Ansibotzone"
                        ),
                    ],
                    [
                        InlineKeyboardButton(
...
""",
            ...
            [
                InlineKeyboardButton(
                    ...
                    [
                        InlineKeyboardButton(text="♥️ вαϲк ♥️", callback_data="Ansi_back"),
                    ],
                ]
           ),
        )
...
def donate(update: Update, context: CallbackContext):
    ...
    if chat.type == "private":
        ...
        if OWNER_ID != 5617001475 and DONATION_LINK:
            update.effective_message.reply_text(
                f"» ᴛʜᴇ ᴅᴇᴠᴇʟᴏᴩᴇʀ ᴏғ {dispatcher.bot.first_name} sᴏᴜʀᴄᴇ ᴄᴏᴅᴇ ɪs [ɢɪᴛʜᴜʙ](https://github.com/BadshahAk/ansi_Ro_bot)"
...
""",
    ...
    about_callback_handler = CallbackQueryHandler(
        Ansi_about_callback, pattern=r"Ansi_"
    )
    ...
    LOGGER.info("💖AnsiRobot♥️ started with fully smoothest power💖")
