...
def paginate_modules(page_n: int, module_dict: Dict, prefix, chat=None) -> List:
    ...
    if len(pairs) > 3:
        pairs = pairs[modulo_page * 10 : 10* (modulo_page + 1)] + [
            (
                EqInlineKeyboardButton(
                    "☜︎︎︎", callback_data="{}_prev({})".format(prefix, modulo_page)
                ),
                EqInlineKeyboardButton(
                    "☯︎ ʜᴏᴍᴇ ☯︎", callback_data="Ansi_back"
                ),
                EqInlineKeyboardButton(
                    "☞︎︎︎", callback_data="{}_next({})".format(prefix, modulo_page)
                ),
            )
        ]

    else:
        pairs += [[EqInlineKeyboardButton("• ʙᴀᴄᴋ •", callback_data="Ansi_back")]]
